...
def draw_reslut(setted_container:Container,filename):
    ...
    for cargo in setted_container._setted_cargos:
        ...
        # 保存图片
    plt.savefig('E:/postgraduate/课程资料/高级算法设计与分析/算法大作业/fig3/' + filename + '.jpg',dpi=600)
    # 重置一下plt
    plt.clf()
...
def _draw_cargo(ax,cargo:Cargo):
    _plot_opaque_cube(
        ax,cargo.x, cargo.y, cargo.z,
        cargo.length, cargo.width, cargo.height
    )

def _plot_opaque_cube(ax,x=10, y=20, z=30, dx=40, dy=50, dz=60):
...
def _plot_linear_cube(ax,x, y, z, dx, dy, dz, color='red'):
    # ax = Axes3D(fig)
    xx = [x, x, x+dx, x+dx, x]
    yy = [y, y+dy, y+dy, y, y]
